...
public class SmashBallController : MonoBehaviour {
    Vector3 WaluigiPos;
	...
	void Start () {
        WaluigiPos = Vector3.zero;
        wayPoint = new Vector3(8.48f, -7.37f, 0);
        exitLocation = new Vector3(8.48f, 3.64f, 0f);
    ...
    bool GoGhost()
    {
        ...
        if (Mathf.CeilToInt(ghostTimer * Mathf.Sqrt(randomNum2)) % randomNum1 == 0)
        {
            if (randomNum2 % 13 == 0)
            {
                ghostDestination.x = WaluigiPos.x;
                ghostDestination.y = WaluigiPos.y;
